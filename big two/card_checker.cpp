...
Card* CardChecker::check_straight(std::vector<Card>* cards)
{
	if (cards->size() != 5)
		return NULL;
	sort_by_value_and_suit(cards);
	if (cards->at(0).get_value() != cards->at(1).get_value() - 1 || cards->at(1).get_value() != cards->at(2).get_value() - 1 || cards->at(2).get_value() != cards->at(3).get_value() - 1 || cards->at(3).get_value() != cards->at(4).get_value() - 1)
		return NULL;
...
Card* CardChecker::ai_select_full_house_turn1(std::vector<Card>* cards)
{
	if (cards->size() < 5)
		return NULL;
	sort_by_value_and_suit(cards);
	if (cards->at(0) != Card(0, 3))
		return NULL;
	//TODO code below accounts for 333XX but not XXX33
	if (cards->size() < 5)
		return NULL;
	Card top_card_copy = top_card;
